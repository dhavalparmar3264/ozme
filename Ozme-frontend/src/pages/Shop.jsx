...
import { Star, Heart, ShoppingCart, Eye, Sparkles, Loader2, Zap } from 'lucide-react';
...
function ShopPage({ onProductClick, onQuickView }) {
  ...
  const handleBuyNow = async (e, product) => {
    e.stopPropagation();
    try {
      // Determine the size to use (default to first available size or '120ML')
      let sizeToUse = '120ML';
      if (product.sizes && Array.isArray(product.sizes) && product.sizes.length > 0) {
        sizeToUse = product.sizes[0].value || product.sizes[0].size || '120ML';
      } else if (product.size) {
        sizeToUse = product.size;
      }

      // Add product to cart
      await addToCart(product, 1, sizeToUse);
      
      // Redirect to checkout after successful add
      navigate('/checkout');
    } catch (error) {
      // Error is already handled by addToCart with toast notification
    ...
    try {
      ...
      if (response && response.success) {
        ...
        const transformedProducts = response.data.products
          .filter(product => product.active) // Only show active products (all products, not just bestsellers)
          .map(product => {
  ...
  // Sort products according to selected filter
  ...
  // Reorder products: First 4 bestsellers, then all other products (including remaining bestsellers)
  // This ensures bestsellers are prioritized while maintaining the sort order within each group
  const bestsellers = sortedProducts.filter(p => p.tag === 'Bestseller');
  const nonBestsellers = sortedProducts.filter(p => p.tag !== 'Bestseller');
  
  // Take first 4 bestsellers, then add all non-bestsellers, then add remaining bestsellers
  const firstFourBestsellers = bestsellers.slice(0, 4);
  const remainingBestsellers = bestsellers.slice(4);
  const reorderedProducts = [...firstFourBestsellers, ...nonBestsellers, ...remainingBestsellers];
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-6 sm:mb-8 pb-4 sm:pb-6 border-b border-gray-200">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 sm:ml-auto w-full sm:w-auto">
                <span className="text-xs sm:text-sm text-gray-600 font-light text-center sm:text-left">
                  {loading ? 'Loading...' : `${reorderedProducts.length} Products`}
                </span>
                  ...
                  {reorderedProducts.map((product) => (
                    <div key={product.id} className="group" onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)}>
                      <div className="relative">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 md:p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                            <button onClick={(e) => handleBuyNow(e, product)}
                              className="w-full py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm bg-white text-gray-900 font-semibold hover:bg-gray-900 hover:text-white transition-all duration-300 flex items-center justify-center gap-1.5 sm:gap-2 shadow-xl">
                                <Zap className="w-3 h-3 sm:w-4 sm:h-4" />Buy Now
                            </button>
                          </div>
                        </div>
                        ...
                        {/* Mobile Buy Now Button - Visible on mobile only */}
